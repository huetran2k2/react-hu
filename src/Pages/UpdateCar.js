...
const UpdateCar = () => {
    ...
    const fetchData = () => {
        return axios.get(`http://127.0.0.1:8000/api/showdetail/${id}`)
            .then((response) => setCar((prevState) => ({
                ...prevState,
    ...
    return (
        ...
        value = { name }
        onChange = {
            (value) => handleOnChange('name', value) }
        /> <
        /div> <
        div className = "form-group" >
        <
        ...
        value = { price }
        onChange = {
            (value) => handleOnChange('price', value) }
        /> <
        /div> <
        div className = "form-group" >
        <
        ...
        onChange = {
            (value) => handleOnChange('mf_id', value) } > {
            manuList.map((manu, index) => {
                return ( <
                    option value = { manu.id }
                    key = { index } > { manu.mf_name } < /option>
                )
            })
        } <
        /select> <
